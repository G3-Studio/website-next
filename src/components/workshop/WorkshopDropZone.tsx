...
import { Component, Subcomponent, Workshop } from '@/types';
import { DragEvent, DragEventHandler, useState } from 'react';

export default function WorkshopDropZone({
  id,
  h,
  workshop,
  drop,
  ...
  function dragEnter(e: DragEvent<HTMLDivElement>) {
    ...
    if (e.target instanceof HTMLDivElement && (allowed[0] == 1 || allowed[1] == 1 || allowed[2] == 1)) {
      (e.target as HTMLDivElement).classList.add('h-8');
      if ((e.target as HTMLDivElement).children[0] instanceof HTMLElement) {
        (e.target as HTMLDivElement).children[0].classList.remove('opacity-0');
      }
    }
  }
  ...
  function dragOver(e: DragEvent<HTMLDivElement>) {
    ...
    if (dropZone.children[0]) {
      ...
      let line = dropZone.children[0] as HTMLElement;
  ...
  function setLine(line: HTMLElement, dropZoneZone: number) {
    if (dropZoneZone == 0) {
  ...
  function getAllowedDropZones(id: string, workshop: Workshop) {
    ...
    if (idArray.length == 3) {
      ...
      let component = workshop.components.find((component: Component) => component.order === parseInt(idComponent));
      ...
      let idComponent = idArray[2];
      let component = workshop.components.find((component: Component) => component.order === parseInt(idComponent));
      if (component === undefined) return [0, 0, 0];
      let subcomponent = component.subcomponents.find(
        (subcomponent: Subcomponent) => subcomponent.order === parseInt(idSubcomponent),
      );
      ...
      if (subcomponent.subsubcomponents.length > 0) {
        ...
        if (
          ...
          component.subcomponents[component.subcomponents.length - 1].order == parseInt(idSubcomponent)
        ) {
          nextComponent = 'component';
        } else {
          nextComponent = 'subcomponent';
      ...
      if (
        subcomponent.subsubcomponents.length > 0 &&
        subcomponent.subsubcomponents[subcomponent.subsubcomponents.length - 1].order === parseInt(idSubsubcomponent)
      ) {
        // if the current subcomponent is the last one, then the next component is a component
        if (
          component.subcomponents.length > 0 &&
    ...
    if (
      nextComponent == 'component' &&
      workshop.components.find((component: Component) => component.order == parseInt(idArray[2] + 1)) === undefined
    ) {
    ...
    if (
      (previousComponent == 'component' && nextComponent == 'component') ||
      (previousComponent == 'component' && nextComponent == 'bottom')
    ) {
      return [1, 1, 0];
    }
    ...
    if (
      (previousComponent == 'subcomponent' && nextComponent == 'subcomponent') ||
      (previousComponent == 'subsubcomponent' && nextComponent == 'subcomponent')
    ) {
      return [0, 1, 1];
    }

    if (
      (previousComponent == 'subsubcomponent' && nextComponent == 'subsubcomponent') ||
      (previousComponent == 'subcomponent' && nextComponent == 'subsubcomponent')
    ) {
      return [0, 0, 1];
    }

    if (
      (previousComponent == 'subsubcomponent' && nextComponent == 'component') ||
      (previousComponent == 'subsubcomponent' && nextComponent == 'bottom') ||
      (previousComponent == 'subcomponent' && nextComponent == 'bottom') ||
      (previousComponent == 'subcomponent' && nextComponent == 'component')
    ) {
      return [1, 1, 1];
    }
  ...
  return (
    <div
      ...
      className={
        'relative  transition-all ease-in-out duration-100 flex justify-center items-center w-full ' + (h ? h : 'h-4')
      }>
