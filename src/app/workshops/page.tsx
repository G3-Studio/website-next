...
export default async function Workshops() {
  const workshops: Workshop[] = (await prisma.workshop.findMany()) as Workshop[];

  return (
    <Layout title="Workshops" session={undefined}>
        ...
        <div className="flex flex-col flex-wrap items-center justify-start m-auto md:flex-row">
          {workshops.map((workshop: Workshop) => (
            <Link
              ...
              key={workshop.id}
              className="flex flex-col h-auto overflow-hidden border rounded shadow-lg lg:flex-row lg:h-32">
              <img
                className="flex-none block w-full h-auto bg-cover lg:w-48"
